...
function weatherHook(shader) {
  ...
  shader.uniforms.uWeatherA = { value: new THREE.Vector3(p.edge, p.groove, p.riv) };
...
function dressHullMats(model, opts) {
  ...
  model.traverse(o => {
    ...
    if (lit) {
      ...
      if (opts.weather) weatherHullMat(m);   // plating rides the hull branch only
    }
  });
}
